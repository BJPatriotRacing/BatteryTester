/*
  ...
  Copyright: 2018-2026 (C) All rights reserved
  ...
  03.0 kasprzak  11/15/2023 changed sliders for RTC to number pad input

  04.5 kasprzak  3/19/2026 added option for 10.5 volt cutoff (non persisted)
...
#include <ILI9341_t3_PrintScreen_SdFat.h>  //https://github.com/KrisKasprzak/ILI9341_t3_PrintScreen
...
//uint16_t ScreenLeft = 3900;
//uint16_t ScreenRight = 350;
//uint16_t ScreenTop = 3900;
//uint16_t ScreenBottom = 270;


uint16_t ScreenLeft = 350;
uint16_t ScreenRight = 3900;


uint16_t ScreenTop = 270;
uint16_t ScreenBottom = 3900;
...
byte BatTemp = 0, BatTempID0= 0, BatTempID1= 0, BatTempID2= 0, BatCutOffID1= 0, BatCutOffID2= 0;
byte Battery= 0, Tester= 0, b= 0, BatteryCutoffValue = 0;  // holders for filename formatting
// strings for the buttons
char str[40];
char buf[40];

char FileName[50] = "BB_YYYY-MM-DD_T_NN.csv";
char BMPName[50] = "BB_YYYY-MM-DD_T_NN.BMP";
...
uint16_t rYear, rMonth, rDay, rHour, rMinute;
...
OptionButton BatteryCutoffLimit(&Display);
...
NumberPad InputNumberPad(&Display, &Touch);
...
void setup() {
  ...
  DownloadBtn.init(210, 78, 180, 56, C_GREY, C_DKGREY, C_WHITE, C_WHITE, "Download", 0, 0, FONT_HEADING);
  CalibrateBtn.init(210, 140, 180, 56, C_GREY, C_DKGREY, C_WHITE, C_WHITE, "Calibrate", 0, 0, FONT_HEADING);
  SetTimeBtn.init(210, 202, 180, 56, C_GREY, C_DKGREY, C_WHITE, C_WHITE, "Set Time", 0, 0, FONT_HEADING);
  ...
  DoneBtn.init(265, 20, 90, 33, C_GREY, C_DKGREY, C_WHITE, C_WHITE, "Save", 0, 0, FONT_HEADING);
  OnBtn.init(265, 220, 90, 33, C_DKGREEN, C_GREEN, C_BLACK, C_WHITE, "ON", 0, 0, FONT_HEADING);
  OffBtn.init(265, 220, 90, 33, C_DKRED, C_RED, C_DKGREY, C_BLACK, "OFF", 0, 0, FONT_HEADING);
  StopBtn.init(280, 15, 60, 29, C_DKRED, C_RED, C_WHITE, C_WHITE, "STOP", 0, 0, Arial_12_Bold);

  SSDEraseBtn.init(95, 70, 180, 38, C_GREY, C_DKGREY, C_WHITE, C_WHITE, "Erase SSD", 0, 0, FONT_HEADING);
  SSDDownloadBtn.init(95, 110, 180, 38, C_GREY, C_DKGREY, C_WHITE, C_WHITE, "Download", 0, 0, FONT_HEADING);
  SetTesterBtn.init(95, 150, 180, 38, C_GREY, C_DKGREY, C_WHITE, C_WHITE, "Tester #", 0, 0, FONT_HEADING);
  ...
  ProfileBtn.setBorderThickness(0);
  DownloadBtn.setBorderThickness(0);
  CalibrateBtn.setBorderThickness(0);
  SetTimeBtn.setBorderThickness(0);
  BatteryBtn.setBorderThickness(0);

  StartBtn.setBorderThickness(0);
  DoneBtn.setBorderThickness(0);
  OnBtn.setBorderThickness(0);
  OffBtn.setBorderThickness(0);
  StopBtn.setBorderThickness(0);

  SSDEraseBtn.setBorderThickness(0);
  SSDDownloadBtn.setBorderThickness(0);
  SetTesterBtn.setBorderThickness(0);
  SSDDoneBtn.setBorderThickness(0);
  ...
  InputNumberPad.init(C_BLACK, C_WHITE, C_BLUE, C_DKBLUE, C_WHITE, C_DKGREY, C_BLACK, FONT_HEADING);

  InputNumberPad.enableDecimal(false);   // disable / enable decimal point (enabled by default)
  InputNumberPad.enableNegative(false);  // disable / enable negative sign (enabled by default)
  InputNumberPad.setDecimals(0);
  InputNumberPad.setClickPin(BUZZ_PIN);
  InputNumberPad.setTouchLimits(ScreenLeft, ScreenRight, ScreenTop, ScreenBottom);
  InputNumberPad.useButtonIcon(true);

  RTCM.init(70, 40 + 20, 90, 33, C_GREY, C_GREEN, C_BLACK, C_WHITE, "Month", 0, 0, FONT_HEADING);
  RTCD.init(70, 75 + 20, 90, 33, C_GREY, C_GREEN, C_BLACK, C_WHITE, "Day", 0, 0, FONT_HEADING);
  RTCY.init(70, 110 + 20, 90, 33, C_GREY, C_GREEN, C_BLACK, C_WHITE, "Year", 0, 0, FONT_HEADING);
  RTCH.init(70, 145 + 20, 90, 33, C_GREY, C_GREEN, C_BLACK, C_WHITE, "Hour", 0, 0, FONT_HEADING);
  RTCI.init(70, 180 + 20, 90, 33, C_GREY, C_GREEN, C_BLACK, C_WHITE, "Minute", 0, 0, FONT_HEADING);

  TestTime.init(10, 120, C_BLACK, HANDLE_COLOR, C_LTGREY, C_BLACK, C_WHITE, 30, 2, "Pre-race test", FONT_SMALL);
  TestTime.value = true;

  BatteryTemp.init(C_BLACK, HANDLE_COLOR, C_LTGREY, C_BLACK, C_WHITE, 20, -2, FONT_SMALL);

  BatteryCutoffLimit.init(C_BLACK, HANDLE_COLOR, C_LTGREY, C_BLACK, C_WHITE, 20, -2, FONT_SMALL);
  BatCutOffID1 = BatteryCutoffLimit.add(20, 90, "10.5 Volts");
  BatCutOffID2 = BatteryCutoffLimit.add(160, 90, "11.0 Volts");

  BatteryCutoffLimit.select(BatCutOffID2);
  ...
  Display.print(100.0 - ((float)TotalTests / TOTAL_TESTS) * 100.0, 0);
  Display.print(F("%"));
  ...
  if (FileCount > 50) {
    ...
    Display.fillRect(20, 20, 280, 200, C_RED);
    Display.setTextColor(C_WHITE, C_RED);
    Display.setFont(FONT_HEADING);

    Display.setCursor(40, 40);
    Display.print(F("Memory is full"));

    Display.setFont(FONT_SMALL);

    Display.setCursor(40, 80);
    Display.print(F("Files in memory: "));
    Display.print(FileCount);

    Display.setCursor(40, 105);
    Display.print(F("Download data to an SD card"));

    Display.setCursor(40, 130);
    Display.print(F("Erase the memory"));

    Display.setCursor(40, 180);
    Display.print(F("Touch screen to continue"));
  ...
  if (BatteryCutoffLimit.value == BatCutOffID1){
    BatteryCutOff = 10.5;
  }
  else {
    BatteryCutOff = 11.0;
...
void GetFileName() {
  ...
  while (FNKeepIn) {
    ...
    if (Touch.touched()) {
      ...
      BatteryCutoffValue = BatteryCutoffLimit.press(BtnX, BtnY);
#endif

      if (PressIt(BatteryBtn) == true) {

        InputNumberPad.setMinMax(1, 99);  // want bound checks?
        InputNumberPad.value = Battery;
        InputNumberPad.getInput();
        Battery = InputNumberPad.value;
...
void DrawFileNameScreen() {
  ...
  BatteryCutoffLimit.draw(BatteryCutoffLimit.value); 
...
void DrawSetTimeScreen() {
  ...
  Display.setCursor(150, 40 + 20);
  Display.print(rMonth);

  Display.setCursor(150, 75 + 20);
  Display.print(rDay);

  Display.setCursor(150, 110 + 20);
  Display.print(rYear);

  Display.setCursor(150, 145 + 20);
  Display.print(rHour);

  Display.setCursor(150, 180 + 20);
  Display.print(rMinute);
}
...
void SetTime() {
  ...
  rYear = year(), rMonth = month(), rDay = day(), rHour = hour(), rMinute = minute();
  ...
  while (CalKeepIn) {
    ...
    if (Touch.touched()) {
      ...
      if (PressIt(RTCM) == true) {
        InputNumberPad.setMinMax(1, 12);  // want bound checks?
        InputNumberPad.value = rMonth;
        InputNumberPad.getInput();
        rMonth = InputNumberPad.value;
        DrawSetTimeScreen();
      }
      if (PressIt(RTCD) == true) {
        InputNumberPad.setMinMax(1, 31);  // want bound checks?
        InputNumberPad.value = rDay;
        InputNumberPad.getInput();
        rDay = InputNumberPad.value;
        DrawSetTimeScreen();
      }
      if (PressIt(RTCY) == true) {
        InputNumberPad.setMinMax(0, 2049);  // want bound checks?
        InputNumberPad.value = rYear;
        InputNumberPad.getInput();
        rYear = InputNumberPad.value;
        DrawSetTimeScreen();
      }
      if (PressIt(RTCH) == true) {
        InputNumberPad.setMinMax(0, 23);  // want bound checks?
        InputNumberPad.value = rHour;
        InputNumberPad.getInput();
        rHour = InputNumberPad.value;
        DrawSetTimeScreen();
      }
      if (PressIt(RTCI) == true) {
        InputNumberPad.setMinMax(0, 59);  // want bound checks?
        InputNumberPad.value = rMinute;
        InputNumberPad.getInput();
        rMinute = InputNumberPad.value;
        DrawSetTimeScreen();
      }
    }
  }
...
void DownloadData(bool SoundWarning) {
  ...
  if (SoundWarning) {
    ...
    Display.print(F("SSD memory FULL"));
    Display.setFont(FONT_HEADING);
    Display.setCursor(10, 130);
    Display.print(F("1. Download to an SD card"));
    Display.setCursor(10, 180);
    Display.print(F("2. Erase SSD chip"));
    SSDDoneBtn.draw();
    analogWriteFrequency(BUZZ_PIN, 200);
    for (i = 0; i < 3; i++) {
      analogWrite(BUZZ_PIN, 100);
  ...
  while (KeepIn) {
    ...
    if (Touch.touched()) {
      ...
      if (PressIt(SetTesterBtn) == true) {
        ...
        InputNumberPad.setMinMax(1, 5);  // want bound checks?
        InputNumberPad.value = Tester;
        InputNumberPad.getInput();
        Tester = InputNumberPad.value;
